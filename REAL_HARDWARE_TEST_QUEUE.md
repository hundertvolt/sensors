...
the body cap and was **rewritten** (§2A F10).

**The post-merge full bench run is DONE (2026-09-19, §7I): 97 passed, 1 failed, 4 skipped, 27
deselected in 40:39** — the reference totals exactly, with the one failure being W5. **The body
cap's own claim is confirmed on silicon under concurrency**: all four of W5's rewritten assertions
hold in the suite and in three dedicated repeats — zero wrong statuses, every run. The red came
from the *post-load health check* one line further down, which was single-shot against a server
whose 24 just-closed slots were still draining; a `GET /status` polled instead succeeds **75 ms**
later. **§1D is now fully closed**: that check was fixed on the owner's decision (§2A **F11**), and
W5 has run green three times since.

**R15 is also closed** by the same sitting (§7I.4), and **F12** is recorded (a hand-issued REST
reboot strands the DUT in hotspot mode unless stations are kicked first).

**The 2026-09-19 evening sitting (§7J) then ran the suite eight more times** — three consecutive
clean default runs, plus two runs with the wear gates open — and closed **F7, F11, F13, F14** and
F15's instance.

**The two host-side rows that sitting left open are now closed too (2026-09-19, no bench time).**
**F15's residual** — the marker guard had no concept of a wrapper it could not recognise — is shut by
three new checks, and fixing it surfaced two live defects in the guard itself (a keyword fallback
looking for `body=` when the parameter has always been `json_body`, and a positional-only body read),
so a keyword-passed PUT body was invisible to every detector. **F16** is root-caused and fixed:
**reproduced 3/3 locally** under CPU load, so it was never CI-only, and *both* recorded leads were
wrong — `limit=300` is `asyncio.wait_for`'s bound in seconds, not event-loop steps, and the listener
budget is not binding (150 of 450 rounds used). The real mechanism is the mock tier's no-CRC
...
joined to the local network as an ordinary device. **`DebugLevel` must be set back to 5 before any
`tests_hardware/` bench-tier run** — several tests parse the serial log and fail silently at 0.
§7J.6 has what was changed and how to undo it.
**Both webserver handovers are deleted** — `REAL_HARDWARE_HANDOVER_POST_MERGE_BENCH_RUN.md` (its
mandate was one full bench run at default flags, done and migrated here) and
`REAL_HARDWARE_HANDOVER_WEBSERVER_BODY_CAP.md` (its own stated condition was W5 coming back green,
which it has). `REAL_HARDWARE_HANDOVER_MEASURE_B.md` is **deleted** — §7H migrated every
...
> **The run overturned this prediction, and the prediction is kept because it was pre-registered.**
> A *did* improve `largest_block` in-suite, by **40.2%** (20,592 → 28,864 B), reproduced
> byte-identically by the bench tier (§7D.3). It was investigated rather than believed, exactly as
> asked, and the cause is neither of the two guessed here: `Board.run_isolated()` never resets, so
> the twin's ensemble measured a **cold** heap, the one configuration in which the defect does not
> exist (§7D.2). §7B.1 and §7C.2 both carry that correction now. The 80,000 B floor this paragraph
> measures against was **retired by the owner on 2026-09-19** (§7G); the replacement checks pass
> with 5.5x margin (§7H.4).
...
| W5 | `test_concurrent_mixed_body_sizes_are_never_answered_with_the_wrong_status` | **DONE 2026-09-19: PASS.** The rewrite's four assertions held on silicon from the first run — in the post-merge bench suite and in three dedicated repeats, **zero requests answered with the wrong status**, zero non-ceiling exceptions, both verdicts present, 16-20 of 24 answered against a floor of 4. The row nonetheless went red on its *post-load health check*, which was single-shot against a server whose 24 just-closed slots were still draining — not a cap result at all (§2A F11). **Fixed on the owner's decision**: that check now uses the same bounded `wait_until()` the neighbouring connection-ceiling test already uses for the identical lag. Three consecutive green runs afterwards (16/8, 19/5, 20/4 answered/refused, 0 wrong each), and the fail direction verified to still bite against both a silent host and a dead port. | DONE |
...
| R14 | **RAN on silicon 2026-09-19 and PASSED first time** (§7H.6), which is why the status cell reads DONE. It was the one host-side fix that could have turned a previously green test red (2026-09-18). It now requires the device script's own `"WDT armed, starving now"` banner in the failure text, so a transient `"may be in use by another program"` at connect can no longer clear the `< 10.0 s` bound and pass as a watchdog reset. If it fails on the next S1, read *which* case: a genuinely absent banner is the fix working (a real connect failure that used to pass silently), while a banner that is merely not captured means mpremote dropped buffered device stdout as the link died - and then the marker is the wrong instrument and the reset needs proving another way. The other four fixes from the same host-side review are settled: `_usb_reset_device()`'s by-id path was confirmed on the bench host, the rest are source-grounded. | **DONE 2026-09-19: PASS** on silicon, first run of the banner assertion. §7H.6 | DONE |
...
| F11 | **W5's post-load health check had no settle and no retry, so it landed inside the connection-slot drain window — and that, not the body cap, was what kept the row red.** Measured 2026-09-19 (post-merge bench run + 3 dedicated repeats): the four assertions the row was rewritten around all pass, reproducibly — **0 answered wrongly**, 0 non-ceiling exceptions, both verdicts present, every run. The failure was the next line, a single-shot `http_client.fetch(..."GET", "/status")`, raising `ConnectionResetError` — the ceiling-refusal signature, since `_serve()`'s reject-when-full branch closes without writing a response. Polled immediately after the load, a `GET /status` succeeded **75 ms** later in 2 of 3 repeats and first-try in the third: 24 workers' slots were still draining through `_serve()`'s `finally`, which runs *after* `_close_writer()` awaits. `WEBSERVER`'s log stayed empty in every run. **RESOLVED 2026-09-19 — owner's decision: "a defect in the test setup, not a failure ... the test must still fail if the server genuinely is unreachable, but under fair circumstances, not within an almost-zero-delay recovery phase where it doesn't even have a chance."** The health check now uses the **same `wait_until()` the neighbouring `test_connections_at_and_above_the_real_socket_limit_degrade_cleanly` already uses 170 lines up for the identical lag** (15 s bound, 1 s poll), whose own comment names this exact phenomenon. Not a weakening: `wait_until()` treats a raising check as not-yet-ready but still raises `TimeoutError` with the last exception embedded once the bound expires — **verified to bite**, host-side, against a silent host (`TimeoutError` ... `No route to host`) and a dead port (`Connection refused`), both inside the bound. All four cap assertions are untouched. | **RESOLVED — fixed and verified** |
| F12 | **A manual `PUT /system {"SystemCmd": "reboot"}` outside the fixtures leaves the DUT in hotspot mode, because nothing kicked the AP's stale station entry.** Hit 2026-09-19 running R15 by hand: the reboot command returned `Valid`, the board reset and came up healthy (all tasks running, every sensor reading, uptime advancing), but STA never re-associated — the serial log showed `WIFI Hotspot mode is active / Connected stations: []` on repeat, and `NTP Network not available`. It stayed that way for ~4 minutes. A `board.hard_reset()` recovered it to 192.168.85.57 within ~40 s. **Not a defect and not new**: `tests_hardware/conftest.py`'s `dut_ip` fixture calls `bench.kick_all_stations()` before *every* `hard_reset()`, and `test_real_reboot_sequencing_via_rest_completes_cleanly` calls it immediately after the same REST reboot, its own comment naming "the same stale-AP-station-table finding as every other real reboot in this tier". That test passed in this very run. The trap is only for a session issuing the reboot by hand: **kick stations first, and expect to need a `hard_reset()`**. Recorded in §6. | RECORDED |
| F13 | **`test_real_sta_connect_reaches_established_after_a_hard_reset` asserted a single trial of a genuinely probabilistic real-RF event, and missed one.** Pass 1 of the 2026-09-19 triple run: `2 failed, 96 passed, 4 skipped, 27 deselected`. Root cause is one failure, not two — the DUT's STA association failed twice after `kick_all_stations()` + `hard_reset()` (`WLAN connection failed` / `WLAN status: -1`, six connect polls each), so the firmware did what it is designed to do and fell back to hotspot; `test_real_ntp_handles_a_genuinely_unreachable_server_without_crashing` then failed with `No route to host`, a pure cascade of the DUT being off the LAN. **Measured before deciding anything**: one `kick_all_stations()` + `hard_reset()` recovered it with zero connect failures, and **12 further cold boots established 12/12 with zero connect failures**, so the reconnect path is not broken — it had passed in the two preceding full runs too, making it 1 miss in 3. `WLAN status: -1` is the CYW43 reporting association failure, i.e. an AP/RF-side event, not a firmware logic error. **Fixed the same way the owner settled F11** — a single association is not "fair circumstances" for a test that must still catch a real break. The row now takes at most **one** second cold boot, asserts unchanged on the last attempt, names the attempt count in both failure messages, and prints a `RESULT NOTE` whenever the retry was needed, so a rising flake rate stays visible instead of being absorbed. A systematic break fails both attempts, which is what the row exists to catch — it is the primary regression coverage for the stale AP-side station-table scenario and that is deliberately not blunted. | **RESOLVED — fixed and verified** |
| F14 | **The first-ever run with the wear gates open found three failures, all in tests that had never executed, and none of them a firmware defect.** `scripts/run_bench_hardware_suite.sh --allow-persistence-writes --allow-scd30-extra-write` (2026-09-19): **3 failed, 117 passed, 4 skipped, 5 deselected in 48:27** — the gates pulled in **22 tests that every prior sitting had deselected**, which is exactly where never-run code shows its first bugs. **(a) `test_same_device_concurrent_sessions_never_corrupt_each_other`**: `CO2=119.73297` on iterations 0-4, *the same value every time*. Not corruption — a stale data register. `bus_concurrency_same_device_scd30.py` calls `scd.setup()` (a soft reset) and reads immediately, while the SCD30's measurement interval is NVM-persisted and so resumes at once, raising data-ready over the first unsettled conversion. **Its sibling `scd30_same_device_rw_concurrency.py` was already fixed for this and its comment names the identical signature** ("a stuck CO2=141.99 on every iteration"); this script never got the same treatment. Fixed by mirroring it exactly — a 12 s drain that *calls* `read_measurement()` rather than sleeping, because data-ready clears only on read. **(b) and (c) `test_isl29125_config_write_...` / `test_bmp3xx_config_write_...`**: `ConnectionResetError` on concurrent writes — F10/F11's slot-release lag a third time. These open only **3** connections, *below* `max_connections = 4`, and the file's own header already tried to avoid this by margin ("must stay under max_connections=4 with real margin ... or a brief overlap ... hits a genuine (but here undesired) reject-when-full"). Margin is not enough: a slot is released in `_serve()`'s `finally` *after* `_close_writer()` awaits, so back-to-back worker requests outrun it. (c)'s second error, `PressOvers=1 rejected: ... "Unchanged"`, is a **cascade** — a lost write desynchronised the worker's alternation, so the next value already matched. Fixed with a bounded retry on a *ceiling close only*; any other exception still raises. The `is_ceiling_close()` predicate was **promoted out of `test_network_resilience.py` into `http_client.py`** per CLAUDE.md Part G's shared-primitive rule rather than duplicated, and W5 now uses the shared one. All three pass; nothing in `src/` is implicated and no bound was moved. | **RESOLVED — fixed and verified** |
| F15 | **The persistence-write marker guard cannot see a PUT issued through a wrapper, and F14's fix demonstrated it by accident.** `tests_scripts/test_persistence_write_marker_completeness.py`'s `_persisting_put_functions()` matches an AST call named exactly `fetch` with **at least 5 positional args**, reading the method from `args[2]` and the body from `args[4]`. F14's first draft introduced `_fetch(dut_ip, "PUT", "/sensors", {...})` — a different name, four positional args, method and body at different indices — which made `isl29125_write_worker` and `bmp3xx_write_worker` **invisible to the guard**, and CI caught it as `gone: ['bmp3xx_write_worker', 'isl29125_write_worker']`. **The guard worked exactly as designed** (it pins the set by name in both directions, so a helper *leaving* the set is as loud as one joining it), and the tempting wrong fix — deleting the two names from `_KNOWN_PERSISTING_HELPERS` — would have made two real persisting writes permanently unwatched. **Fixed by restoring visibility, not by excusing it**: the retry wrapper is now named `fetch` with `http_client.fetch`'s exact positional signature, so the detector sees straight through it and `_KNOWN_PERSISTING_HELPERS` is **unchanged**. A comment on the wrapper says the name and signature are load-bearing. **RESIDUAL CLOSED 2026-09-19, host-side, no bench time.** Three checks were added, each proven by injecting the regression it exists for. (1) `test_no_fetch_wrapper_hides_its_method_from_the_detector` fails on any `fetch()` call that passes its method as a variable - F14's draft exactly - with a `_JUSTIFIED_FORWARDED_METHODS` allowlist holding one entry, the retry wrapper itself. (2) `test_every_forwarding_wrapper_still_mirrors_fetchs_own_signature` re-derives that entry's premise, comparing the wrapper's positional parameter names against `http_client.fetch`'s own, so a rename or a reorder fails instead of being excused by a stale allowlist line. (3) `test_fetch_is_the_only_route_from_this_tier_to_an_http_request` pins `fetch()` as this tier's single HTTP chokepoint, closing the wrapper-that-never-calls-fetch case. **Two live defects in the guard surfaced while doing it**: its keyword fallback looked for `body=` when the parameter has always been `json_body`, so that escape hatch could never fire, and `_persisting_put_functions()` read `args[4]` positionally only - together leaving a keyword-passed literal PUT body invisible to both detectors. Both fixed by deriving fetch's real argument positions and body keyword from its own `def` (`_fetch_shape()`) rather than hardcoding 2 and 4. | DONE |
| F16 | **`tests/test_uart_comm_hazard.py`'s clean-hammer rows inherit a 30 ms wall-clock protocol deadline the host cannot always meet under `scripts/test.sh`'s own oversubscription.** CI run `35468454090` saw `only 149/150 hammered transactions completed`. **ROOT-CAUSED AND FIXED 2026-09-19, host-side.** **Reproduced 3/3 locally** by running the full file under 24 CPU hogs on 4 cores - so it is *not* CI-only, and `test.sh` creates this condition deliberately (4x the core count, 16 concurrent interpreters, plus the backgrounded `tests_scripts/` tier). **Both recorded leads were wrong, and were checked rather than assumed**: `limit=300` is `asyncio.wait_for`'s timeout in *seconds*, not an event-loop step budget - an expiry would raise `TimeoutError`, never return 149 - and the listener budget is not binding either, measured at exactly 150 of 450 rounds consumed. The real mechanism is `ready()`'s wall-clock deadline: the loopback link delivers in-memory within one event-loop turn, so the configured `timeout` can only ever measure whether the *host* rescheduled in time. Sized against J.6's floor the no-CRC arm sat at **1.25x** (30 ms over 24 ms) where CRC16 sat at 10x and the real `dev` link at 13.3x; measured max scheduling gap was 3 ms idle, 23-26 ms at 8x oversubscription, past 30 ms when loaded. Fixed by building the two tests that assert a *clean* run completes in full (`_hammer_clean`, `_measure_retention`) at `_TIMEOUT_MS * 8`, the CRC arm's existing margin - a clean run never consumes the timeout, so it costs no wall clock, and the short budget stays where it makes recovery cycles cheap. Derivation and the accepted trade-off: SPECIFICATION.md Part J.7. | DONE |
...
- **The connection-slot lag cuts both ways — a check made immediately *after* a burst is as
  exposed as one made immediately before it.** F10 explained the *pre*-load half (the
  `reset_all_error_logs()` PUT still holding a slot); F11 was the same mechanism *after* the load,
...
- **A REST reboot issued by hand strands the DUT in hotspot mode; the fixtures kick the AP's
  ...
  `hard_reset()` afterwards (which recovered it in ~40 s). §2A F12.
