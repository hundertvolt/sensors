...
| W8 | **Confirm the committed limit of 6** (image E6′ = the tip): identical to E6, peak load without instrumentation (3 boots), exact peak heap (3 boots), refusal cross-check host = device, 8 clients against the limit, `/status` timing, the full bench tier — `REAL_HARDWARE_HANDOVER_LIMIT_6.md` | OPEN |
