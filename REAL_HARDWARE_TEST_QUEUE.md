...
deselected in 40:39** — the reference totals exactly, with the one failure being W5 again. **The
body cap's own claim is now confirmed on silicon under concurrency**: all four of W5's rewritten
assertions hold in the suite and in three dedicated repeats — zero wrong statuses, every run. The
red comes from the *post-load health check* one line further down, which has no settle and no
retry and lands in the connection-slot drain window; a `GET /status` polled immediately after
succeeds **75 ms** later. That is §2A **F11**, and it needs a decision, not bench time.

**R15 is also closed** by the same sitting (§7I.4), and two new traps are recorded — F11 above and
**F12** (a hand-issued REST reboot strands the DUT in hotspot mode unless stations are kicked
first).

What is still owed: **F11's decision**, D1-gated work, the remaining R-rows, §1C's C2/C3/C4, and
F1's SSID script. `REAL_HARDWARE_HANDOVER_POST_MERGE_BENCH_RUN.md` is **deleted** — its mandate
was one full bench run at default flags, which is done and migrated here.
`REAL_HARDWARE_HANDOVER_WEBSERVER_BODY_CAP.md` stays open on W5 alone, now narrowed to F11's one
line. `REAL_HARDWARE_HANDOVER_MEASURE_B.md` is **deleted** — §7H migrated every
...
| W5 | `test_concurrent_mixed_body_sizes_are_never_answered_with_the_wrong_status` | **RE-RUN 2026-09-19 post-merge: every one of the four rewritten assertions HOLDS on silicon — and the row still goes red, one line further down.** In the bench run and in three dedicated repeats, all four pass identically: no non-ceiling exception, **zero wrong statuses**, 20 of 24 answered (floor is 4), both verdicts present. What fails is the *post-load health check* that follows them — `GET /status` gets `ConnectionResetError`, reported as "webserver unresponsive after concurrent mixed-body load". It is not unresponsive: polled immediately, it answers 200 **75 ms later** (2 of 3 repeats reset once then succeeded; the third succeeded first try). Same slot-release lag F10 already root-caused, at the *other* end of the test — W5 takes a settle **before** its workers and none **after** them, where 24 connections just closed. `WEBSERVER`'s log stayed **empty** throughout. **Reported, not changed** — see §2A F11 | **Body-cap claim CONFIRMED on silicon; red on an unrelated health check** |
...
| F11 | **W5's post-load health check has no settle and no retry, so it lands inside the connection-slot drain window — and that, not the body cap, is what keeps the row red.** Measured 2026-09-19 (post-merge bench run + 3 dedicated repeats): the four assertions the row was rewritten around all pass, reproducibly — 20 of 24 answered, 4 refused at the ceiling, **0 answered wrongly**, 0 non-ceiling exceptions, both verdicts present, every run. The failure is the next line, `assert http_client.fetch(..."GET", "/status").status_code == 200`, which raises `ConnectionResetError` from `http.client._read_status` — the ceiling-refusal signature (`_serve()`'s reject-when-full branch closes without writing a response). Polling immediately after the load, a `GET /status` succeeded **75 ms** later in 2 of 3 repeats and first-try in the third, so the server is not unresponsive; 24 workers' slots are still draining through `_serve()`'s `finally`, which runs *after* `_close_writer()` awaits. `tests_hardware/http_client.py`'s `fetch()` is single-shot by construction — no retry — and W5 takes its `time.sleep(1.0)` settle **before** the workers and none **after** them. `WEBSERVER`'s error log stayed **empty** in every run, so nothing in `src/` is implicated. **Flagged, not edited** (CLAUDE.md, and the handover's own "no weakening of W5"). The obvious fix is the one the neighbouring `test_connections_at_and_above_the_real_socket_limit_degrade_cleanly` already uses at line 646 for this exact lag — a settle, or a bounded retry, on the health check only, leaving all four cap assertions untouched. Owner's call. | OPEN — needs a decision, not bench time |
...
- **The connection-slot lag cuts both ways — a check made immediately *after* a burst is as
  ...
  `reset_all_error_logs()` PUT still holding a slot); F11 is the same mechanism *after* the load,
  where 24 workers' slots are draining through `_serve()`'s `finally`. A single-shot
  ...
  unresponsive", poll it** — the drain window is tens of milliseconds, not seconds.
