...
**Where things stand (2026-09-19).** §1A (measure A) is **run and closed** — §7D. §1B (measure B)
is **run but partial** — §7F, headline settled (the tripwire passes in-suite for the first time).
**§1C is untouched**: features that landed with the merge and have hardware implications neither
§1A nor §1B covers.
...
| # | Run | Notes | Status |
| --- | --- | --- | --- |
| B1 | **The matched in-suite pair**, A-only against A + B. | **DONE 2026-09-18 — P1 CONFIRMED.** BEFORE (A only) `free=105,088 largest_block=28,736` **FAIL**; AFTER (A+B) **PASS**, suite `36 passed, 3 skipped, 12 deselected`, zero failures. First in-suite pass on any image ever measured. Results: §7F. **One thing still owed and now free**: the exact AFTER `largest_block`. `test_memory_stress.py` prints every `HEAP ` line on a *passing* run since 2026-09-18, so `scripts/run_flash_hardware_suite.sh -s` surfaces it with no extra invocation — and those lines now carry `retained=`, which is what would flag §7F.8's artefact had it struck the tripwire | DONE, one figure owed |
| B2 | **The whole boot sequence, not just `build_system()`.** `device_scripts/heap_layout_after_full_boot_sequence.py`. | **PARTIAL — run 2026-09-18 at the fresh-heap position, with an instrument that could not see the thing P2 is about.** What stands: `starters=22 timers=8` matching §7E, and **P3 confirmed** (BEFORE 89% → 70% across the starter list). What does not: the `after_starter_list` rows on both arms are taken 4 s into the **run phase** (§7F.9), and the `..._production_threshold` row is a **probe artefact** and is withdrawn (§7F.8). **P2 is untested, not refuted** — and the script now takes the reading that tests it. See B6 | PARTIAL |
...
| B7 | **The replaced tripwire's first run** (owner retired the 80,000 B floor 2026-09-19). No new invocation — it is what S1 already runs. | **OPEN.** Two of its four checks are safe against everything ever measured (`used <= 100,000` against a measured 87,760-87,968; `largest_free_run >= 32,768` where every A+B reading cleared 80,000). The two **placement** checks have never been measured on any image. The **delta** one — the boot places nothing in the top 16,384 B — is the owner's "zero survivors in the long heap", and it is the one to trust: it held in 17 of 17 twin runs across 41-58% fill, fresh and aged, perturbed (§7G.5), and it is independent of suite position by construction. The **absolute** one can fail on what `main.py` left behind rather than on the boot. **A failure of either is a finding, not automatically a regression** — report the `MAP` and `DELTA` lines, and do not re-fit a threshold to make it pass. `highest_new_pct` on the `DELTA` line is the real margin: the twin gives 84-87% at board-like fill, so the top 13-16% is clear there | OPEN |
