...
| --- | --- | --- | --- |
| D1 | **Does the bench spend flash/NVM writes this round?** A default run deselects 12 of the bench tier's 71 tests and 9 of the flash tier's 51; `--allow-persistence-writes` runs them and spends real cycles. `--allow-scd30-extra-write` is AND-gated on top for one further SCD30 NVM write. | Several rows below are *only* reachable with the flag — R1 and R4 in particular. Deselection is invisible to the pass/fail check, so this must be a knowing choice (CLAUDE.md's wear rule). | OPEN |
| D2 | **Is the NeoPixel-aimed-at-the-ISL29125 rig set up, and should the two long lighting tests be gated on it?** This branch runs `test_isl29125_survives_recombined_realistic_lighting_scenarios` (~8.5 min of real light programs) and `test_isl29125_mechanism_envelope_holds_across_range_resolution_and_calibration` (~99 s of settles) **ungated**; `main` gates both behind `@pytest.mark.neopixel_sweep` + `--allow-neopixel-sweep` with the skip reason "needs the NeoPixel-aimed-at-the-sensor rig physically set up". Both sides are real; they disagree. | An ungated run fails outright if the LED is not aimed at the part, and costs ~10 min when it is. The 2026-09-17 bench session ran them ungated and they passed after the `_park()` fix, which suggests the rig *is* set up — confirm rather than assume. Resolving this is also one of the 30 `main` merge conflicts. | OPEN |
