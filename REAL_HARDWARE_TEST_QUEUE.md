...
**Board state**: `dev` image of tree `dd80eef` (the NTP no-give-up fix included), `buildDate
2026-09-25T05:21:43Z`, `max_connections = 6`, `DebugLevel` 5. **Behind the tree again**: `b5450aa`
changed `asy_wifi_service.py` (radio byte bounds, `PERIODIC` hotspot timer), so reflash before W4's
re-run. Check `buildDate` against the tree every time.
