...
F15's instance. What is still owed: **F15's residual blind spot** and **F16** (a single undiagnosed
CI-only `test_uart_comm_hazard` failure), D1-gated work, the remaining R-rows, §1C's C2/C3/C4, and
F1's SSID script.
...
| F15 | **The persistence-write marker guard cannot see a PUT issued through a wrapper, and F14's fix demonstrated it by accident.** `tests_scripts/test_persistence_write_marker_completeness.py`'s `_persisting_put_functions()` matches an AST call named exactly `fetch` with **at least 5 positional args**, reading the method from `args[2]` and the body from `args[4]`. F14's first draft introduced `_fetch(dut_ip, "PUT", "/sensors", {...})` — a different name, four positional args, method and body at different indices — which made `isl29125_write_worker` and `bmp3xx_write_worker` **invisible to the guard**, and CI caught it as `gone: ['bmp3xx_write_worker', 'isl29125_write_worker']`. **The guard worked exactly as designed** (it pins the set by name in both directions, so a helper *leaving* the set is as loud as one joining it), and the tempting wrong fix — deleting the two names from `_KNOWN_PERSISTING_HELPERS` — would have made two real persisting writes permanently unwatched. **Fixed by restoring visibility, not by excusing it**: the retry wrapper is now named `fetch` with `http_client.fetch`'s exact positional signature, so the detector sees straight through it and `_KNOWN_PERSISTING_HELPERS` is **unchanged**. A comment on the wrapper says the name and signature are load-bearing. **Residual, recorded not fixed**: the guard still has no concept of a wrapper — it has `_JUSTIFIED_UNREADABLE_BODIES` for a body it cannot read, but nothing for a call it cannot recognise, so the next wrapper that is not shaped like `fetch` reopens the same hole silently. Resolving one level of intra-module indirection, or asserting that no `tests_hardware/` module defines a PUT-issuing wrapper the detector would miss, would close it. Owner's call — it is host-side work needing no bench time. | OPEN (record) — the instance is fixed, the blind spot is not |
| F16 | **`tests/test_uart_comm_hazard.py` failed once in CI with `only 149/150 hammered transactions completed`, and has not been reproduced locally.** CI run `35468454090`, job `unit-tests`: `tests/test_*.py (MicroPython Unix port): 84/85 files passed`, the one file being this; `tests_scripts/` passed in the same run. **Not caused by this sitting's work** — every change here is in `tests_hardware/` or docs, and this is the mock tier, which shares no code with it; the three preceding CI runs on the same branch were green. `_hammer_clean()` asserts all 150 of `_HAMMER_ROUNDS` complete through an in-memory fake pair under `run(hammer(), limit=300)`. **Re-run locally 5 times on the same Unix-port binary: 96/96 passed every time** (a 12-run characterisation was cut short when the sitting was paused). So it is CI-runner-specific so far, exactly the shape the repo already documents for this tier (CLAUDE.md's `select.poll()` and heapsize entries), but **it is not diagnosed and must not be written off as a flake on 5 local runs**. Next step is more local runs and, if it stays unreproducible, reading whether `limit=300` is a tight bound on the event-loop steps 150 transactions plus a 450-round listener actually need — a slower runner would then lose the last transaction, which is precisely the observed 149/150. | OPEN — undiagnosed, CI-only |
