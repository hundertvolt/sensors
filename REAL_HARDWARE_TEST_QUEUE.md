...
| W5 | **The static-file fix on silicon** (image F′, the tip): the same file, now probing `route:/` and checking every body against its `Content-Length`. Twin: `route:/` needs 336 B (1,536 B before), sweep 4/6/8/10 clean with every body complete. `REAL_HARDWARE_HANDOVER_CONNECTION_SCALING.md` §00 | OPEN |
| W6 | Per-boot combined load on image F′ at N = 2/4/6/8, bodies checked — the sitting's §10.6 definition. Twin: all complete, zero failures | OPEN |
