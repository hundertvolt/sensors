...
| W7 | **Free heap under full load at 4/5/6/7**, image F′, `gc.threshold(-1)`: per-boot `combined_load_sweep.py 4 5 6 7`, then the same with `--margin` (post-collect free heap per sample, idle vs under load, % of GC heap). Owner is choosing a limit below 10. Twin estimate on F′: 6 ≈ 20 %, 7 ≈ 15 %, 8 ≈ 11 % — `REAL_HARDWARE_HANDOVER_FREE_HEAP.md` | OPEN |
