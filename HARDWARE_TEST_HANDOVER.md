...
| **W3** | Like for like on one tree (2026-09-25): `/status` (6.86 KB) median **1.29 s at 256 B** (1.11–1.46 s) against **1.10 s at 1,024 B** (0.92–1.25 s), 20 idle samples each — the 256 B cap costs **+0.19 s (+17 %)**; `/networking` and `/measurements` fit one piece and are unchanged (~0.28 s). Needed a throwaway 1,024 B image (2 firmware flashes, owner-authorised) | Owner: accept +17 % as the price of the bound, or not |
...
| **R1** | Gated run: both config-write arms passed with `CEILING_RETRIES` "none" — but only 4 writes, which a 33 % rate passes by chance ~20 % of the time. **Dedicated repeat of the original shape** (one config-persisting `PUT /sensors {"ISL29125": {"Resolution": 12/16}}` against 2 concurrent `GET /sensors` readers, 18 attempts, 1.5 s apart): **18/18 ok** in 0.53–1.88 s, no reset, no ceiling refusal, readers 82/82 ok, error logs clean afterwards. The same shape failed 6/18 on 2026-09-17; a clean 18 at that rate is ~0.07 % likely. *Wear*: 18 config writes. **BACKLOG 30 closed as not reproduced — row retired** | None. If it ever returns, its signature (RST in 21–72 ms, nothing logged) and the `RangeAuto=false` bisection are in this commit's history |
...
| **G12** | New `device_scripts/uart_driver_read_never_blocks_the_loop.py` + `test_the_shipped_driver_never_holds_the_loop_for_a_frame_still_arriving` (flash tier): the shipped `readinto_until_complete()` reads a 53-byte frame over the jumper while its own UART calls are timed through a proxy. **Longest driver call 126-132 us**; the control, an unclamped read of the same frame, 3,251 us; bound 1,533 us (a third of the wire time). Passed 7 of 7 runs on silicon. A loop-gap probe is printed but not asserted: it read 1.6 ms usually and 2.9 ms once, scheduler noise (idle ticker alone ~590 us), as F.5.8 warns. Figure added to SPECIFICATION F.5.8. **Done — row retired** | None |
| **G8** | Already covered: `test_serving_heap_at_default_gc.py::test_serving_sweep_at_the_reactive_default` imports the full `sensortask_dev` task graph at `gc.threshold(-1)`, sweeps concurrent requests past the ceiling and asserts no `MEMORY_ERROR_MARKERS` — G8's bar exactly. It passed on silicon twice today (W4 re-run, step 6). **Row retired** | None |
| **G3** | New `device_scripts/reboot_fallback_starves_the_watchdog.py` + `test_reboot_falls_back_to_starving_the_watchdog_when_its_timer_cannot_arm` (flash tier): a real `SystemService` with a real 1.5 s watchdog, the alarm pool exhausted (**16 timers**), then `_reboot()`: the reset timer could not arm, `_force_watchdog_starve` was set, `feed_watchdog()` was called 6 times over 1.25 s without feeding, and the watchdog reset the board (`reset_cause()` = 3, `WDT_RESET`); the reboot action never fired. 3/3 runs. **Done — row retired** | None |
...
6. Step 9's scripts: ~~G8~~ (covered), ~~G12~~ (done), ~~G3~~ (done), ~~G1~~ and ~~G4~~
   (**scratched** — each needed a `src/` change for the test alone, owner 2026-09-25).
   ~~R1~~ — repeated, 18/18 clean, BACKLOG 30 closed (5.2).
   ~~W3's 1,024 B comparison~~, ~~R6/R7~~ — done after the owner authorised the throwaway flashes
   (5 firmware flashes; the standard `12:54:13Z` image is back on the board).
