...
- **GET shapes**:
  ...
  - `/system` → flat settings only: `DebugLevel, TaskCheckSecs, GMTOffset, DSTOffset`.
...
- **PUT shapes** — one sparse JSON body per endpoint, no `cmd` envelope: any field present is
  ...
  - `/system` — `{<any subset of DebugLevel,TaskCheckSecs,GMTOffset,DSTOffset>, "SystemCmd":
...
| `system_service.py` (`"SYSTEM"`) | 1-6 | dynamic (`n + 1`) | 4=task-error-budget-exceeded-rebooting, 5=`_log_dead_task()` recovering a dead task's real exception (Task has no `.exception()` in MicroPython's asyncio — awaiting the finished task re-raises it) before the routine per-index `wrnno` restart warning below; a task that returned cleanly (the documented restart contract — see Part C.4.2's `read_loop()` note) never reaches errno=5, only one that raised. 6=`_log_dead_task()` recovering a task that ended via `asyncio.CancelledError` — previously logged via the non-persisting `PrintLog.err()` (no `self.pr` of its own history involved), so this case left **zero** trace in `get_error_counter()`, indistinguishable from a clean return; fixed to persist via its own errno so a self-cancelled task is no longer invisible next to a real crash (errno=5) or a genuine clean return (neither). `wrnno` assigned per task-supervisor index — see the "dynamic assignment" bullet above, not a fixed catalog. **Real-world instance (bench hardware, confirmed via this exact fix)**: a `wrnno=10` restart (task index 9 — `NotificationCoordinator.start_asy_auto_override`) was reproducible on nearly every real boot, self-healing immediately, `get_error_counter()` showing only two persisted `wrnno=10` entries and nothing else. Before this fix, that was a real contradiction — `auto_led_override()`'s only clean-return path (the `_finalized` guard) was independently confirmed `True` at every entry via a live diagnostic, ruling out a clean return, while `err_count` never exceeding the number of `wrn_s()` calls ruled out both `_start_task()`'s own errno=3 (the initial start never failed) and `_log_dead_task()`'s own errno=5 (no real exception). The one remaining, then-invisible possibility — the task ending via `CancelledError` — is exactly what this errno=6 fix makes observable. Also added: `TaskCheckSecs` (config_SYSTEM.cfg, live-settable via `PUT /system` like `DebugLevel`, no reflash needed) overrides the task-supervisor's own poll cadence (default matches `_TASK_CHECK_TIME`=2s) for a tighter diagnostic feedback loop, and `_log_dead_task()` gained an `all()`-gated (`DebugLevel`=5) task-identity/heap (`id()`/`gc.mem_free()`) snapshot — both landed specifically to chase this bug and are generically useful for any future task-restart investigation. |
