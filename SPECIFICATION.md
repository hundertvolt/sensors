...
| `asy_bmp3xx_driver.py` (`BMP3XX`) | 10-22 | — | 10=init, 11=periodic read (and ONLY that: it covered the calibration leg too until 2026-09-16, the one number any promoted driver used at two sites, which made an `E11` in a field log unattributable between a dead read path and a best-effort calibration giving up - free numbers left here are 23, 36, 37), 12=config read at init, 13=config write at init, 14=config read at store-time, 15-20=oversampling/filter forwards, 21=trigger-interval, 22=batched snapshot read. |
