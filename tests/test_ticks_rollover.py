...
def _discover_this_rigs_ticks_period() -> int:
    # time.ticks_add()'s accepted-delta range is exactly [-period/2, period/2), confirmed against
    # extmod/modtime.c - so this bisects for the largest accepted positive delta rather than assuming a
    # delta of the whole period is itself valid, which the self-consistency test below shows it is not.
