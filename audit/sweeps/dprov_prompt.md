...
WHY. The owner asked to find statements presented as decided, accepted or owner-directed whose real decider is not traceable. Trigger case, corrected (OR64): SPECIFICATION.md C.8 "Known structural gap, accepted risk (`SGP40_I2C._reset()`)" was written by a session agent (`f5c9f90`, 2026-09-03) with no owner trace — yet the owner DID take that decision (in a session, never recorded as the owner's words). So a missing owner trace does NOT prove the owner did not decide; it proves the repo cannot tell owner decisions from agent ones. Classify by the trail you can see ("no owner trace"), never assert that the owner did not decide. The owner's rule (OR48.a (2)): an agent's acceptance is not an owner decision, so untraceable decisions go to the owner for confirmation.
