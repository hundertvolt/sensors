...
The metric is the **reach**: the byte offset of the highest block newly allocated by a boot list,
relative to the top of what was already allocated when that list began ("the seam"). Negative means
the allocation went into a hole *below* the seam.

| position | metric | collects live | collects suppressed | separation |
|---|---|---|---|---|
| setup batch | reach above seam, worst of 6 devices | **278,688** | 2,103,008 | 7.5x |
| setup batch | median above seam, worst of 6 | **45,056** | 1,174,880 | 26x |
| setup batch | new blocks > 512 KiB above seam | **0** on every device | 1,110–1,678 | — |
| both lists | reach above seam, worst of 6 | **633,344** | 3,530,208 | 5.6x |
| both lists | median above seam, worst of 6 | **−55,744** (below the seam) | 1,213,632 | — |

Three facts that matter for the board run:

- **The reach is heap-size independent.** The same code measured **byte-identical** reach at
  `-X heapsize=8M` and `16M` (1,046,688 live / 3,336,064 suppressed in the calibration pair), while
  `largest_free_run` as a fraction of free moved from 96% to 93% for the same arm. That is why no
  fill calibration is needed and why the host bounds are absolute offsets.
...
**B9 — the ratio, which is the transferable claim.** Compare B7 and B8's reach. The host says 7.5x
(batch) and 5.6x (both lists). A board ratio in the same direction and order of magnitude confirms
the mechanism on silicon; a board ratio near 1.0 would mean the mechanism does not hold at the
board's own fill, which is a finding either way and would need recording before anything else.

**B10 — hold the twin's qualitative claim up to the board.** The host's live arm puts the *median*
new block at or below the seam top on every device. Check whether the board does the same. This is
the strongest single statement the metric makes and the one least dependent on units.

## 4. What transfers to the board and what does not

- **Does not transfer: any absolute byte bound.** §1's numbers are twin units — 32 B blocks, x86-64
  words, and every allocation figure inflated 4–5x by `MICROPY_PY_SYS_SETTRACE` being compiled into
  the test binary (§1.2 item 7, §3A). The host test says so in its own constants' comment, and the
  board's tripwire stays the hardware test. **Do not copy 640 KiB or 1,280 KiB onto the board.**
