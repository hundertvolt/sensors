...
_DEFAULT_CONFIG = const("{\"LedAutoOn\": true, \"LedWifiOn\": true, \"LedAutoInterv\": 300, \"LedAutoOnH\": 10, \"LedAutoOnM\": 0, \"LedAutoOffH\": 18, \"LedAutoOffM\": 0, \"LedAutoFlashDur\": 2, \"LedAutoFlashBri\": 200, \"LedWarnCO2\": 1600, \"LedWarnVOC\": 350, \"LedWarnHum\": 65, \"BMPTempOvers\": 1, \"BMPPressOvers\": 1, \"GMTOffset\": 3600,  \"DSTOffset\": 3600, \"BMPTempOffset\": 0, \"BMPPressOffset\": 0, \"BMPSampleInterv\": 2, \"BMPFiltCoeff\": 0, \"BMPSeaLevelOffs\": 0, \"BMPMeanAtmTemp\": 15, \"NTP_Host\": \"pool.ntp.org\", \"SSID\": \"\", \"Hostname\": \"SensorNode\", \"PW\": \"\", \"NTP_Offset_S\": 0, \"Country\": \"DE\", \"NTP_Interv_H\": 12, \"SGPBackupPeriod\": 1, \"SGPBackupMaxAge\": 7200, \"SGPWaitTimeNTP\": 30}")
...
_BMP388_Pressure = const(0)
_BMP388_Temperature = const(1)
_BMP388_SeaLevelPressure = const(2)
_BMP388_Timestamp = const(3)
...
async def bmpCfgCallback():
    return await cfgmgr.get_values(["BMPSampleInterv", "BMPPressOvers", "BMPPressOffset",
                                    "BMPTempOvers", "BMPTempOffset", "BMPFiltCoeff",
                                    "BMPSeaLevelOffs", "BMPMeanAtmTemp"])
...
fram = asy_FRAM_manager(spi0, 1, max_size=0x2000, debug=debug)
...
bmp_reader = BMP3xx_Reader(i2c1, bmpCfgCallback, max_i2c_err=_MAX_I2C_ERR, debug=debug)
...
async def sensor_status(request):
    ...
    bmp_meas = await bmp_reader.get_data()
    meas_data = {
        "SCD30": {
        ...
        "BMP388": {
            "Pres": bmp_meas[_BMP388_Pressure],
            "Temp": bmp_meas[_BMP388_Temperature],
            "SLPres": bmp_meas[_BMP388_SeaLevelPressure],
            "TS": bmp_meas[_BMP388_Timestamp]
        }
    }
    return meas_data
...
async def sensor_config(request):
    ...
    (valid, bmp388_conf) = await cfgmgr.get_json(["BMPSampleInterv", "BMPPressOffset", "BMPTempOffset", "BMPSeaLevelOffs", "BMPMeanAtmTemp"])
    try:
        bmp388_conf["BMPPressOvers"] = await bmp_reader.get_pressure_oversampling()
        bmp388_conf["BMPTempOvers"] = await bmp_reader.get_temperature_oversampling()
        bmp388_conf["BMPFiltCoeff"] = await bmp_reader.get_filter_coefficient()
    except:
        valid = False

    if not valid:
        bmp388_conf = {
            "BMPSampleInterv": "None",
            "BMPPressOvers": "None",
            "BMPPressOffset": "None",
            "BMPTempOvers": "None",
            "BMPTempOffset": "None",
            "BMPFiltCoeff": "None",
            "BMPSeaLevelOffs": "None",
            "BMPMeanAtmTemp": "None"
            }
...
async def sensor_cmd(request):
    ...
    if req_json["cmd"] == "setBMP":
        if debug: print("Received Set BMP3xx Sensor command.")
        res, err = await init_json_from_cfg(cfgmgr, ["BMPSampleInterv", "BMPPressOvers", "BMPTempOvers",
                                                     "BMPFiltCoeff", "BMPPressOffset", "BMPTempOffset",
                                                     "BMPSeaLevelOffs", "BMPMeanAtmTemp"])
        if res is None:
            return err
        ...
        res = update_valid_json(req_json, "BMPPressOvers", "int", res, 0, 5, weight_fct=lambda x : 2 ** x, debug=debug)
        ...
        res = update_valid_json(req_json, "BMPSeaLevelOffs", "float", res, -1000.0, 5000.0, debug=debug)
...
async def system_status(request):
    ...
    ErrorStatus = ( (sgp_fram_crit > 0) or (sgp_fram_uncrit > 0) or (SCD30_ErrCnt > 0) or
                    (BMP388_ErrCnt > 0) or (SGP40_ErrCnt > 0) or (Task_ErrCnt > 0) )
    system_data = {
...
async def main():
    ...
    task_starters.append(bmp_reader.start_asy_read)
    task_starters.append(bmp_reader.start_asy_trigger)
    ...
    timer_starters.append(bmp_reader.start_timer)
