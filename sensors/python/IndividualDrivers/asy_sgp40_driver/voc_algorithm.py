...
class DFRobot_vocalgorithmParams:
    ...
    def unpack(self, bytearr):
        try:
            values = struct.unpack("31q", bytearr)
        except:
            return False
        (self.mvoc_index_offset,
         self.mtau_mean_variance_hours,
         ...
         self.m_adaptive_lowpass_x1,
         self.m_adaptive_lowpass_x2,
         self.m_adaptive_lowpass_x3) = values
        return True
...
class VOCAlgorithm:
    ...
    def vocalgorithm_proc_ser_des(self, sraw: int, serialize=False, deserialize=None):
        deserialized = None
        if deserialize is not None:
            deserialized = self.params.unpack(deserialize)
        voc_index = self.vocalgorithm_process(sraw)
        bytearr = None
        if serialize:
            bytearr = self.params.pack()
        return voc_index, bytearr, deserialized
