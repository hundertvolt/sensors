...
def generate_module_source(model: DeviceModel, construction_order: "list") -> str:
    ...
    sensor_vars = [ctx.instance_var(spec.key) for spec in sorted(instances.values(), key=lambda s: construction_order.index(s.key)) if spec.driver_info and spec.driver_info.kind == "sensor"]
