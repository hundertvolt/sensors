...
def _emit_header_and_imports(lines: "list[str]", model: DeviceModel, ctx: _Ctx, instances: "dict[tuple[str, str], InstanceSpec]", have: "set[str]") -> None:
...
def generate_module_source(model: DeviceModel, construction_order: "list[str | tuple[str, str]]") -> str:
    ctx = _Ctx(model)
    ...
    lines: list[str] = []
    _emit_header_and_imports(lines, model, ctx, instances, have)
