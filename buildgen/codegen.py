...
from buildgen.gc_policy import DEFAULT_GC_POLICY, PRESSURE_MODULE, check_pressure_policy, threshold_for
...
def _emit_header_and_imports(lines: "list[str]", model: DeviceModel, ctx: _Ctx, instances: "dict[tuple[str, str], InstanceSpec]", have: "set[str]", build_date: str, gc_policy: str) -> None:
    ...
    # Deliberately NOT underscore-private and not a const(): tests_hardware/ reads this off the
    # frozen image to learn what the build declares, independently of whether main.py has run yet
    # (SPECIFICATION.md Part I.6). A live gc.threshold() read cannot answer that question during boot.
    lines.append(f"BUILD_GC_POLICY = {gc_policy!r}")
    lines.append("")
...
def generate_module_source(model: DeviceModel, construction_order: "list[str | tuple[str, str]]", build_date: str, gc_policy: str = DEFAULT_GC_POLICY) -> str:
    ctx = _Ctx(model)
    ...
    lines: list[str] = []
    _emit_header_and_imports(lines, model, ctx, instances, have, build_date, gc_policy)
...
def generate_boot_entry_source(device: str, gc_policy: str = DEFAULT_GC_POLICY, *, memory_pressure: bool = False) -> str:
    check_pressure_policy(gc_policy, device, memory_pressure=memory_pressure)
    module = f"sensortask_{device}"
    threshold = threshold_for(gc_policy, device)
    lines = [
        f'"""Generated by buildgen: real firmware entry point for the {device} device - blocks forever,',
        'matching how a real deployed unit boots today (see modules/_boot.py)."""',
        "",
        "import asyncio",
        "import gc",
        "",
    ...
    if memory_pressure:
        lines.append(f"import {PRESSURE_MODULE}")
    lines += [
        f"from {module} import main",
        "",
        f"gc.threshold({threshold})",
        "",
    ]
    if memory_pressure:
        # The churn task has to live in the loop main() blocks on: the live system is observed
        # passively over serial, and any exec()-based route interrupts an asyncio.run() that never
        # resumes (tests_hardware/README.md). Started before main() so pressure covers boot too.
        lines += [
            "async def _main_under_pressure() -> None:",
            f"    {PRESSURE_MODULE}.start()",
            "    await main()",
            "",
        ]
    entry = "_main_under_pressure()" if memory_pressure else "main()"
    lines += [
        "try:",
        f"    asyncio.run({entry})",
        "finally:",
