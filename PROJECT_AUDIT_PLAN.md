...
| OR35 | 2026-09-26 | Log flooding: "Check errors and warnings (errno / wrnno) for risks of flooding the logfiles with the same error allover and thereby driving other evidence out of the logs." — Recorded; interpretation pending the owner's answers. Starting point: each history is a ten-slot ring per logger holding codes only (no timestamps, one shared `ErrCount`, `src/print_log.py:158-178`); C.7.1's repeat rule (SPECIFICATION.md:1917-1930) is applied by six modules today (FRAM manager, UART, WIFI, NTP, NOTIFY, SGP40 `W13`), each with its own episode flag, and the SGP40 `W13` flood (`83c9920`) was found only on the bench. Related: `XCUT.T07`, `PERF.T10`, lens L-DIAG, `SENS.S28`. |
