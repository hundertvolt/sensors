...
| OR47.c | 2026-09-26 | Owner revised OR47.a (3): "Concerning the SCD30 interrupt recovery, I need to revert my decision and base it differently: use the value the legacy code used - as far as I remember, it was chosen 500ms as to be shorter than the shortest regular sampling interval of the scd30. As it is a check timer only, it does not cause any bus traffic except in a desynced situation - usually only once and extremely rare." — Verified: legacy used 500 ms with the same 2·`trigger_sec` threshold (`python/IndividualDrivers/asy_scd30_driver.py:61, 77, 141-145`), and `src/` still does (`asy_scd30_driver.py:143, 212-222`), so parity already holds and no code changes; the datasheet's shortest measurement interval is 2 s (range 2-1800 s, `datasheets/` SCD30 Interface Description 1.4.3), so 500 ms is shorter; each tick only reads the RDY GPIO (`irq_pin.value()`), and the bus is touched only when the pin has stayed high for `trigger_sec` s (`trigger_sec = 3` on every device, `devices/*.toml`), i.e. after a missed interrupt. Replaces OR47.a (3): the SCD30 check tick stays at 500 ms with its threshold and counter as they are; it is not a bus-accessing timer, so it gets no bus slot in the stagger plan and its sub-second phase does not count against the minimum-separation guarantee. SPECIFICATION.md C.9.1's SCD30 paragraph (`SPECIFICATION.md:2286-2298`) stays, corrected and completed rather than removed: the tick is a GPIO-only check (reason: shorter than the shortest 2 s interval, legacy value), the rare forced read lands on a tick moment and is serialised by the bus lock like any collision, and the regular SCD30 reads follow the chip's own clock through RDY, so they drift against the RP2040 timers and no stagger can separate them — accepted, bus lock covers it. The rest of OR47.a (3) stands (bus-timer stagger anchored to one starting point, minimum separation, bus-aware placement, proof). OR47.b stays in force at 500 ms: the recovery path is still unproven end to end, so its four-tier validation applies to the unchanged value (threshold 2·`trigger_sec` ticks; the unit test pinning `period == 500` stays correct). The value and its reason join OR48's legacy-divergence record as a "same as legacy, reason recorded" entry. |
| OR47.d | 2026-09-26 | Owner: "do not change the call and setup infrastructure of the scd30 interrupt recovery at all. it is good as it is." — Frozen, code and behaviour: `SCD30_Reader.__init__`'s recovery members (`irq_pin`, `base_trigger_event`, `start_trigger_timer`, `trigger_half_sec`, `read_event`, `scd_timer_triggers`, `asy_scd30_driver.py:140-145`), `start_timer()` (500 ms periodic timer plus the RDY rising-edge IRQ, `:212-225`), `stop_timer()`, `start_asy_init()` and `scd_init_irq()` (`:208-210, 413-421`), the counter reset in the read path, and the starter lists that hand them to the system (`get_task_starters()`/`get_timer_starters()`, `:227-231`). OR47.c's "no bus slot" is therefore a statement for the stagger analysis only, not a change to how the SCD30 starter is registered or called. No audit item changes this path, including cross-cutting ones (OR42.c SCD30 persistence, OR46.b constructor grouping, OR47.a boot phases and the reworked sequencer): each must leave it untouched, and one that cannot goes to the owner first. Allowed around it: tests (OR47.b, with its fault injection in the twin chip model and in the flash device script, never in the driver), and documentation (SPECIFICATION.md C.9.1 per OR47.c). Not part of the frozen path: the FRAM chunk setup `await self.pr.setup()` in `_init_scd()` (`:161-164`), which OR47.a (2) moves into the boot `setup()` batch. |
