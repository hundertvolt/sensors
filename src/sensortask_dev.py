...
# Grouped, generator-fillable constants - same values as sensortask_wozi.py (no per-variant
# re-tuning; owner-confirmed these stay uniform until the automated per-variant generator lands).
_MAX_MODULE_ERROR = const(5)  # consecutive-failure-streak threshold shared by every module's own
# _error_check() give-up decision.
...
# Bare module-level globals (owner-confirmed, same convention as sensortask_wozi.py): every
# long-lived object build_system() constructs is reachable the same way. None until build_system()
# runs - importing this module alone constructs nothing.
...
def _collect_error_sources() -> "list[Any]":
    # Generic fan-in (SPECIFICATION.md Part C.14), mirrors sensortask_wozi.py's own
    # _collect_error_sources() exactly - same object graph, different pins. Every constructed
    # module's own get_error_sources() is the authoritative list for that module.
...
def _collect_level_setters() -> "list[Callable[[int], None]]":
    # Same generic-fan-in shape as _collect_error_sources() above, mirrors sensortask_wozi.py's own
    # _collect_level_setters() exactly - every constructed module's own get_loggers() names every
    # PrintLogHistory it owns.
    assert conn is not None and ntp is not None and fram is not None and sysfunct is not None
    assert sgp_reader is not None and bmp_reader is not None and scd_reader is not None
...
async def build_system(
    *, cfg_path: str = "", debug: int | None = None, web_host: str = "0.0.0.0", web_port: int = 80,
    ...
    # SPECIFICATION.md Part A.7). SGP40 sits on i2c1 here (shared with SCD30), not i2c1-alone as
    ...
    # FRAM chunk 5. BMP3xx sits alone on i2c0 here (wozi: shares i2c1 with SGP40).
    ...
    # Direct (source, field) references (SPECIFICATION.md Part C.14), mirrors sensortask_wozi.py's
    # own registrations exactly.
    ...
    webserver = WebserverService(
        app,
        # Deliberately no fram= here (stays a plain in-RAM PrintLog, not FRAM-backed) - same
...
def _collect_timer_starters() -> "list[Callable[[], None]]":
    ...
    # _collect_task_starters()'s own already-uniform pattern above - same shape as
...
async def main(*, cfg_path: str = "", debug: int | None = None, web_host: str = "0.0.0.0", web_port: int = 80) -> None:
    ...
    # Force an initial sync attempt before the ntp task itself even starts - matches
    # sensortask_wozi.py's own main(): ntp_force_sync() only sets an event flag asy_ntp_time()
    # watches for, so pre-setting it here is equivalent to (and simpler than) waiting for the task
    # to start first.
