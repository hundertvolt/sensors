...
def wet_bulb_temperature(temperature, humidity):
    ...
    if (-20.0 <= temperature <= 50.0) and (0.5 <= humidity <= 99.0):
        Tw = temperature * \
            ...
            0.00391838 * math.pow(humidity, 3.0/2.0) * \
...
def dew_point(temperature, humidity):
    ...
    if (-40.0 <= temperature <= 50.0) and (0.1 <= humidity <= 100.0):
        ...
        dp = toffs * ((coeff1 * temperature) * coeff2 + loghum) / ((coeff1 * toffs) * coeff2 - loghum)
    return dp

def altitude_baro(p0, dh, tmean):
...
def abs_humidity(temperature, humidity):
    if (temperature is None) or (humidity is None):
        return None
    ah = None
    if (-30.0 <= temperature <= 40.0) and (0.0 <= humidity <= 100.0):
        if temperature >= 0.0:
            ...
            a = 7.6
            b = 240.7
        ah = 13.23454 * humidity / (temperature + 273.15) * math.pow(10.0, (a * temperature) / (b + temperature))
    return ah

def rel_humidity(temperature, abs_humidity):
    if (temperature is None) or (abs_humidity is None):
        return None
    rh = None
    if (-30.0 <= temperature <= 40.0):
        if temperature >= 0.0:
            ...
            a = 7.6
            b = 240.7
        rh = abs_humidity * (temperature + 273.15) / (13.23454 * math.pow(10.0, (a * temperature) / (b + temperature)))
