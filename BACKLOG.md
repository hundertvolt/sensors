...
- **Full `src/README.md` promotion-checklist pass against `asy_wifi_service.py` (owner-requested:
  ...
  - **Real, verified discrepancy found and fixed: `_VAL_HOST`'s schema allowed a 1-63 character
    `Hostname`, but `network.hostname()`'s real documented limit is 32 characters**
    ...
    can't crash the task, but it was getting mis-attributed as `hw_op_failed` (errno=12/13, "a WLAN
    hardware fault") rather than recognized as a config-validation gap - meaning a 33-63 char
    configured hostname would make *every* connect/hotspot-activate attempt "fail" for a reason that
    has nothing to do with the WLAN hardware, eventually tripping `wlan_connect()`'s own
    repeated-hardware-failure give-up/restart path (errno=17) for what's really a bad config value.
    **Not this promotion's own bug originally**: `_VAL_HOST`'s schema deliberately mirrored
    `sensortask-wozi.py`'s existing REST-route bound (`update_valid_json(req_json, "Hostname", "str",
    res, 1, 63, ...)`), which still allows the same 33-63 range today - this pass verified the bound
    against real source, found it was already wrong before this promotion touched it, and initially
    flagged rather than silently narrowed it per `src/README.md` section 1 (narrowing changes what
    hostnames the schema accepts, a real behavior change). **The project owner then explicitly
    directed the fix** ("adapt the schema to fit the actual value") - `_VAL_HOST`'s max is now 32,
    closing the gap at its source; two boundary tests added (32 chars accepted, 33 chars falls back
    to default, `tests/test_asy_wifi_service.py` 135 -> 137). `sensortask-wozi.py`'s own REST-route
    bound (still 1-63 there) remains a separate, still-open mismatch - left alone since that file is
    outside this session's promotion scope, not silently carried along.
  - **Cross-branch documentation gap found (flagged, not fixed)**: this file's own module docstring
    ...
    scenario `src/README.md` section 14 and CLAUDE.md's working agreements call out. **Project owner
    confirmed this is expected to resolve on its own once the two branches are merged together** -
    no action needed here now; not re-pointed at a guessed future path or otherwise worked around in
    the meantime.
