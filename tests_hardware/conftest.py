...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line("markers", "over_provisioned_image: only meaningful on a deliberately over-provisioned build (REAL_HARDWARE_HANDOVER_CONNECTION_SCALING.md 5, image B) - informational marker, not skip-gated, so it reports the board's own wall whenever it is run")
