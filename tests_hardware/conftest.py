...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    parser.addoption(
        ...
        help=(
            ...
            "is effectively unbounded for this project's write rates. Without this flag a full "
            "bench/flash run spends zero such writes. Skipped by default. Every test that spends one "
            "at all needs this flag, including the one further gated behind "
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line("markers", "persistence_write: at least one real limited-endurance persistence write (SCD30 on-chip NVM, or the RP2040 flash filesystem behind any config-persisting PUT), directly or via a fixture it depends on - deselected unless --allow-persistence-writes is passed")
