...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line("markers", "over_provisioned_image: only meaningful on a deliberately over-provisioned build (HEAP_FRAGMENTATION_MEASUREMENTS.md §7R.2, image B) - informational marker, not skip-gated, so it reports the board's own wall whenever it is run")
