...
7. **Versioning** — firmware + website, both starting at "2.0b0".
   ...
   **Session 7 done.** The plan's own scope line was one sentence with no further design already
   ...
   1. **Where firmware version surfaces**: a real frozen-bytecode constant (`_FIRMWARE_VERSION =
      const("2.0b0")`), embedded by `buildgen.codegen` into every generated device module exactly
      like `_MAX_MODULE_ERROR`, and reported live through `GET /status`'s `system` section
      (`"FirmwareVersion"`, added to `_system_status()`'s returned dict right after
      `BootSignature`). Runtime-readable, not build-time-only: this is genuinely operational data (a
      fleet operator can ask "which build is this specific physical unit running" without
      re-flashing to check) the same way `BootSignature`/`SysUptime` already are - a build-time-only
      stamp would have answered a different, less useful question ("what did I intend to build"),
      not "what is actually running right now". A fixed, generator-owned catalog entry, not a
      per-device TOML field or a `# @web`-style tag - same category and precedent as
      `buildgen.codegen._KNOWN_SIGNALS` and `buildgen.definitions._status_section()`'s own existing
      live-readonly field lists (SysUptime/BootSignature/LocalTime/UtcTime), since a version isn't a
      per-device fact any one driver file owns. The matching `definitions.json` field def
      (`{"key": "FirmwareVersion", "label": "Firmware Version", "kind": "readonly"}`) was added to
      `buildgen.definitions._status_section()`'s `system_fields` in the same position, so it renders
      on the website's existing Status page through the already-generic field-rendering pipeline
      (`js/templates.js`'s `buildField()` via `resolveFieldValue()`) with **zero new frontend code**.
   2. **Where website version surfaces**: a new top-level `websiteVersion` key in `definitions.json`
      (sibling to `schemaVersion`, generated by `buildgen.definitions.generate_definitions()`) -
      build provenance only, deliberately **not** rendered anywhere in the UI. Reasoned, not
      arbitrary: the Status page's own stated design intent is *live* device state (H.4/H.5's own
      framing), and website version has no live-data dimension the way firmware version does - a
      person loading the page is always looking at exactly the build they just fetched; there's no
      "is this the version I think it is" question to answer live the way there is for a physical
      device that might not have been reflashed yet. Firmware and website versions are tracked as
      **two genuinely independent constants** (both starting at "2.0b0" today, per the plan's own
      literal wording), not one shared value: Session 4/6 already decoupled per-device website
      generation from firmware generation into separate build steps, so a website-only fix (a copy
      change, a UI tweak) can legitimately bump one without forcing an unrelated firmware rebuild to
      claim the same new version number - collapsing them into one value would either force
      artificial lockstep bumps or make the single number ambiguous about which artifact it actually
      describes.
   3. **Single source of truth**: a new module, `buildgen/version.py` (`FIRMWARE_VERSION`,
      `WEBSITE_VERSION`) - both `buildgen.codegen` and `buildgen.definitions` import it directly,
      never duplicated by hand. Deliberately **not** a device TOML field (a version is a per-*build*
      fact, not a per-device one - the same reasoning that already put `_KNOWN_SIGNALS` and the
      Status section's fixed catalogs in the generator rather than the schema); **not**
      `toolchain/versions.toml` (that file pins *external dependency* versions - MicroPython, the ARM
      toolchain - this project consumes, a different kind of "version" than this project's *own*
      release identity; sharing a file risked a future contributor misreading a product-version bump
      as a MicroPython-compatibility statement or vice versa); **not** a `pyproject.toml` field
      (dev-tooling config only, per CLAUDE.md's own explicit "stays frozen-bytecode-only, not
      restructured into an installable package" boundary - the shipped product is never
      `pip`-versioned at all). `buildgen/version.py` fits the same "one small hand-maintained fact
      buildgen owns because nothing else can derive it" shape `buildgen/buildspec.py` already
   ...
   5. **Test coverage, both tiers plus digital-twin reachability**: `tests_scripts/`
      (`test_buildgen_generate.py` - the generated module embeds `_FIRMWARE_VERSION` exactly once
      and reports it in `_system_status()`, all 6 real devices; `test_buildgen_definitions.py` - the
      generated `definitions.json` stamps `websiteVersion` and declares the `FirmwareVersion` Status
      field, all 6 real devices; `test_buildgen_version.py` - the two constants themselves) plus
      `tests/test_sensortask.py` (the real MicroPython Unix-port interpreter tier - `GET /status`
      genuinely returns a non-empty `FirmwareVersion` string, parametrized across all 6 real
      devices via that file's existing per-device scenario mechanism; can't cross-check the exact
      value against `buildgen` from inside the interpreter, since `buildgen` is host-CPython-only
      tooling, so this proves presence/shape the same way every other status field in that same test
      already does). No dedicated digital-twin test was added: `tests/
      test_digital_twin_generated_boot.py`'s existing REST smoke test already asserts a real `GET
      /status` returns 200 for every real device plus both synthetic fixtures, which already
      exercises this code path with no changes needed there.
   ...
   **The two hand-written `html/definitions/wozi.json`/`dev.json` files were updated by hand** (not
   ...
   add the same `websiteVersion` top-level key and `FirmwareVersion` status field, keeping
   `tests_scripts/test_buildgen_definitions.py`'s existing golden-file comparison
   (`generate_definitions()`'s output for wozi/dev must equal these two files exactly) passing
   without weakening that test. `mockdata/wozi.json`/`dev.json` (the `tests_js/`/browser-preview
   mock status fixtures) each gained a `"FirmwareVersion": "2.0b0"` value in their `status.system`
   block for the same reason - `websiteVersion` needed no mock-data change since it lives in
   `definitions.json` itself, which these two files don't model.
   ...
   **`js/definitions.js` gained no new strict validation for `websiteVersion`** - a deliberate
   ...
   back into confusion later.

   **Verification**: `scripts/lint.sh`/`scripts/typecheck.sh` report zero findings; the full
   `tests_scripts/` pytest suite (999 passed, 15 pre-existing skips) and the full real-MicroPython-
   interpreter suite (`scripts/test.sh`, built from scratch in this session's own sandbox - outbound
   apt access was available this time - 55/55 `tests/test_*.py` files passed, `test_sensortask.py`
   itself 315/315) both ran clean end to end, not deferred to CI. `npm test` (vitest, the full
   `js/`/`tests_js/` suite against the real built `wozi` website) ran clean too. This session's own
   changes touch `buildgen/` and `tests_scripts/` (new/edited Python) but not
   `pyproject.toml`/`scripts/`/`toolchain/versions.toml` themselves, so CLAUDE.md's two-target
   clean-chroot pre-push verification recipe does not apply here (that gate is specifically for
   dev-tooling/build-environment setup changes; this session added a new module inside the already-
   in-scope `buildgen/` package and updated JSON/JS/Markdown, the same category of change every prior
   `buildgen/`-touching session already validated through the ordinary sandbox run, not the chroot
   recipe).

   **Not built here** (flagged, not silently absorbed): a version-bump script/process (item 4
   above); rendering `websiteVersion` anywhere in the UI (item 2 above - a deliberate, reasoned
   omission, not a gap); retiring/touching `toolchain/versions.toml` (a different, unrelated kind of
   version, confirmed deliberately kept separate - item 3 above).
