...
def test_resolve_driver_real_drivers(src_dir: Path, driver: str, module: str, class_name: str, kind: str, needs_setup: bool):
...
def test_resolve_driver_unknown_driver_raises(src_dir: Path):
    with pytest.raises(BuildError, match="unknown driver"):
        resolve_driver("nonexistent_chip", src_dir, "dev")


def test_resolve_driver_bmp3xx_naming_convention_is_not_a_naive_uppercase(src_dir: Path):
...
def test_resolve_driver_file_with_no_reader_subclass(tmp_path: Path):
...
def test_resolve_driver_override_table_missing_file(tmp_path: Path):
    with pytest.raises(BuildError, match="does not exist"):
        resolve_driver("fram", tmp_path, "dev")


def test_service_drivers_frozenset_matches_override_table():
...
def test_parse_name_constant_real_drivers(src_dir: Path, driver: str, expected_name: str):
    info = resolve_driver(driver, src_dir, "dev")
    assert parse_name_constant(info.source_path, "dev", driver) == expected_name


def test_parse_name_constant_confirms_notify_is_not_notification(src_dir: Path):
...
def test_parse_name_constant_missing(tmp_path: Path):
...
def test_parse_name_constant_non_string(tmp_path: Path):
    path = tmp_path / "asy_foo_driver.py"
