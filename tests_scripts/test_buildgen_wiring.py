...
def test_parse_wiring_wifi_led_target_is_setter_mode(src_dir: Path):
...
def test_parse_wiring_no_wiring_declared_returns_empty(tmp_path: Path):
    path = tmp_path / "asy_plain_driver.py"
    path.write_text("class Plain_Reader:\n    pass\n")
    assert parse_wiring(path, "dev", "plain") == ()


def test_parse_wiring_not_a_tuple(tmp_path: Path):
...
def test_parse_wiring_malformed_tuple_shapes(tmp_path: Path, body: str, match: str):
    path = tmp_path / "asy_bad_driver.py"
...
def test_parse_wiring_syntax_error(tmp_path: Path):
...
def test_parse_wiring_accepts_attribute_class_reference(tmp_path: Path):
